...
// to Lab colorspace. This test validates our conversion code against the
// colormath one.
TEST(ColorTest, RGBAToLabAWithAtariColors) {
  for (uint32 i = 0; i < kNTSCColors; ++i) {
    double laba[4] = { 0 };
    RGBAToLabA(reinterpret_cast<const uint8*>(kAtariNTSCABGRColorTable + i),
               laba);
    EXPECT_NEAR(kAtariNTSCLabColorTable[(i * 4) + 0], laba[0], 0.01);
    EXPECT_NEAR(kAtariNTSCLabColorTable[(i * 4) + 1], laba[1], 0.01);
    EXPECT_NEAR(kAtariNTSCLabColorTable[(i * 4) + 2], laba[2], 0.01);
    EXPECT_NEAR(kAtariNTSCLabColorTable[(i * 4) + 3], laba[3], 0.01);
  }
}
...
TEST(ColorTest, Ciede2kSharmaWuBalalTestData) {
  ...
  const double lab_1[kSWBTestDataCount * 4] = {
  ...
  const double lab_2[kSWBTestDataCount * 4] = {
  ...
  const double expected_results[kSWBTestDataCount] = {
  ...
  for (size_t i = 0; i < kSWBTestDataCount; ++i) {
    double result = Ciede2k(lab_1 + (i * 4), lab_2 + (i * 4));
    EXPECT_NEAR(expected_results[i], result, 0.01);
  }
}
